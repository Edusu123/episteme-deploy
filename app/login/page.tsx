...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await signIn("credentials", { email, password, redirect: false }).then((res: SignInResponse | undefined) => {
      if (!res) {
        alert("No response!");
        return;
      }
    ...
    <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form onSubmit={handleSubmit} className="space-y-6" action="#" method="POST">
          <div>
            <label className={`block text-sm/6 font-medium ${resolvedTheme === "dark" ? "text-gray-100" : "text-gray-900"}`}>E-mail</label>
            <div className="mt-2">
              <Input
          ...
          <div>
            <div className="flex items-center justify-between">
              <label className={`block text-sm/6 font-medium ${resolvedTheme === "dark" ? "text-gray-100" : "text-gray-900"}`}>Senha</label>
